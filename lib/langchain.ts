...
const fetchMessagesFromDB = async (docId: string) => {
  ...
  await connectDB();
  const messages = await Message.find({ fileId: docId, userId: user.id }).sort({
    createdAt: 1,
  });

  const humanMessage = messages.map((msg) =>
    msg.role === "user"
  ...
  console.log(
    "humanMessage:=> ",
    humanMessage.map((msg) => msg.content.toString())
  );
  return humanMessage;
};
...
export const gerenerateEmbeddingsInPineConeVectorStore = async (
  docId: string
) => {
  const user = await authenticatedUser();
  if (!user.id) {
  ...
  try {
    ...
    const namespaceExists =
      existingNamespaces.namespaces && existingNamespaces.namespaces[docId];
    ...
    console.log(
      `---Storing the embeddings in nameSpace ${indexName} Pinecone vectore store----`
    );
    try {
